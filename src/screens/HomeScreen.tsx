...
import { View, Text, Animated, PanResponder, useWindowDimensions, TouchableOpacity, StyleSheet } from 'react-native';
...
export const HomeScreen = ({ navigation }: { navigation: any }) => {
  ...
  return (
    <Container safe style={{ flex: 1, backgroundColor: '#020306' }}>
      <View style={{ backgroundColor: '#005500', padding: 8, zIndex: 9999, alignItems: 'center' }}>
        <Typography style={{ color: 'white', fontWeight: 'bold' }}>STEP NEXT — MINIMAL NAV LIVE</Typography>
        <Typography style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11, marginTop: 4 }}>
          Acesso à navegação · App Home Container Shell
        </Typography>
      </View>
      
      <View style={{ padding: 24, flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Typography style={{ color: '#00F2FF', fontSize: 24, fontWeight: '800', marginBottom: 16 }}>
          CORE BOOT EFETUADO
        </Typography>
        
        <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, textAlign: 'center', marginBottom: 40, lineHeight: 22 }}>
          Todas as hooks e subscrições Zustand estão ativas em base.{'\n'}
          A cache biográfica foi carregada.{'\n\n'}
          Nenhuma superfície pesada (AI, métricas parciais) está montada no ecrã para atestar isolamento de renderização e navegação pura.
        </Text>

        <TouchableOpacity 
          onPress={() => navigation.navigate('Temas')}
          style={{ width: '100%', padding: 16, backgroundColor: 'rgba(0, 242, 255, 0.1)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(0, 242, 255, 0.3)', alignItems: 'center' }}
        >
          <Typography style={{ color: '#00F2FF', fontSize: 16, fontWeight: '700', letterSpacing: 1 }}>TESTAR TAB: TEMAS</Typography>
        </TouchableOpacity>
        
        <TouchableOpacity 
          onPress={() => navigation.navigate('Dados')}
          style={{ width: '100%', padding: 16, backgroundColor: 'rgba(255, 255, 255, 0.05)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.1)', alignItems: 'center', marginTop: 16 }}
        >
          <Typography style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, fontWeight: '600', letterSpacing: 1 }}>TESTAR TAB: DADOS</Typography>
        </TouchableOpacity>
      </View>
    </Container>
  );
