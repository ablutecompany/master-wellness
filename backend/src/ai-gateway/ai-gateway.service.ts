...
export class AiGatewayService {
  ...
  async generateInsightsV2(
    ...
    const prompt = [
      ...
      ``,
      `REGRAS OBRIGATÓRIAS:`,
      `1. TÍTULO DA SÍNTESE: O título (summary.title) tem de ser curto, interpretativo e específico aos sinais da leitura (ex: "Energia em esforço e recuperação limitada", "Ajustes alimentares como foco prático"). Proibido usar "Leitura Pronta" ou "Análise de Bem-Estar". O título tem de refletir a área em foco (indicada no payload como nextFocus).`,
      `2. SÍNTESE DO MOMENTO: A síntese deve obrigatoriamente mencionar e assumir a dimensão indicada em "nextFocus.dimensionId" (ou a de menor score) como o FOCO PRINCIPAL. Deve citar expressamente os DRIVERS CONCRETOS que causam esse estado (ex: "influenciada sobretudo pelo sono curto, stress elevado e recuperação contextual baixa"). A síntese TEM DE SER COERENTE com o foco final indicado. Se o foco for "food_adjustments", a síntese fala de alimentação/sódio/hidratação. Se for "energy", fala de energia/sono/stress. Proibido: texto genérico, "Resumo pendente OpenAI", frases que servem para qualquer pessoa. Nunca uses a expressão "em um estado" (usa "num estado"). LIMITAÇÕES LINGUÍSTICAS: Usa a palavra "dimensão" raramente. É ESTRITAMENTE PROIBIDO usar as expressões "esta dimensão", "desta dimensão", "a dimensão dos Ajustes Alimentares", "reduziu a margem desta dimensão".`,
      `3. REGRAS POR ESTADO:`,
      `   - Estável: tom tranquilo; reforçar manutenção com base no contexto; não exagerar; não dizer "perfeito".`,
      `   - Atenção: tom prudente; indicar o que merece acompanhamento; sugerir ajuste simples; não alarmar.`,
      `   - Prioritário: indicar prioridade prática; sugerir repetir leitura ou reduzir carga. Se persistir ou houver sintomas, sugerir ponderar contacto com profissional de saúde. Nunca usar "diagnóstico", "hipernatremia", "patologia" ou "desequilíbrio eletrolítico".`,
      `4. FONTES (sourcePolicy): "used" = pode usar. "missing" = indicar ausência se relevante. "excluded_by_user" = nunca usar dados dessa fonte, reduzir confiança ou explicar limite.`,
      `5. REFERÊNCIAS: referencesIntro deve ser exatamente "Entre outras, esta avaliação considerou:". references devem ser 1 a 3 linhas compreensíveis, centradas no fator concreto e impacto ("fator: explicação"). Não repetir "entre outras" em cada referência. Não usar labels crus. Se houver Sódio Urinário elevado, referir em linguagem humana como "Sódio urinário: surgiu elevado nesta leitura e merece acompanhamento. Teve peso na recomendação...".`,
      `6. AÇÕES: 2 a 4 ações por dimensão, específicas, coerentes com estado. Proibido devolver arrays vazios ou fallbacks genéricos como "Sem ações específicas. Manter consistência." ou "Continuar como está" para estados de Atenção/Prioritário. Se houver sódio urinário elevado, incluir ações de moderar sal ou reforçar hidratação. Sem diagnóstico, sem suplementação prioritária.`,
      `7. HISTÓRICO/VITALIDADE: Se history.available = false, não inventar tendência e indicar que a leitura é limitada. Vitalidade não deve roubar foco principal em cenários momentâneos sem histórico longitudinal claro.`,
      `8. DEMO/CONVIDADO: Se isDemo = true, escrever como simulação mas variar conteúdo. Se não houver histórico, confiança mais baixa e explicar que se baseia apenas na sessão.`,
      ``,
