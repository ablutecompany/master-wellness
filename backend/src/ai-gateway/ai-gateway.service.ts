...
export class AiGatewayService {
  ...
  async generateInsightsV2(
    ...
    const prompt = [
      ...
      ``,
      `REGRAS OBRIGATÓRIAS:`,
      `1. TÍTULO DA SÍNTESE: O título (summary.title) tem de ser curto, interpretativo e específico aos sinais da leitura (ex: "Energia em esforço e recuperação limitada", "Ajustes alimentares como foco prático"). Proibido usar "Leitura Pronta" ou "Análise de Bem-Estar".`,
      `2. SÍNTESE DO MOMENTO: A síntese deve obrigatoriamente mencionar e assumir a dimensão indicada em "nextFocus.dimensionId" (ou a de menor score) como o FOCO PRINCIPAL. Deve citar expressamente os DRIVERS CONCRETOS que causam esse estado (ex: "influenciada sobretudo pelo sono curto, stress elevado e recuperação contextual baixa"). Proibido: texto genérico, "Resumo pendente OpenAI", frases que servem para qualquer pessoa. Nunca uses a expressão "em um estado" (usa "num estado").`,
      `3. REGRAS POR ESTADO:`,
